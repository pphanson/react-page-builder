...
class Dropzone extends Component {
  ...
  _unmarkDragElements = () => {
    ;[].forEach.call(
      this.canvasRef.current.querySelectorAll(styles['drag-item']),
      (e) => e.classList.remove(styles.before, styles.after)
    )
  }
  ...
  _addElement = (updatedData) => {
    ...
    this.canvasRef.current.classList.add(styles.dropping)
    setTimeout(() => {
      this.canvasRef.current &&
        this.canvasRef.current.classList.remove(styles.dropping)
    }, 500)
  ...
  render() {
    ...
    return (
      <div
        ref={this.canvasRef}
        className={`${!spaceAvailable ? styles['no-space'] : ''} ${
          id === 'root' ? styles.canvas : ''
        } ${!droppedElements.length ? styles.empty : ''} ${styles.dropzone}`}
      ...
      >
        ...
        {!droppedElements.length ? (
          <p className={styles['dropzone-placeholder']}>{placeholder}</p>
        ) : null}
      </div>
    )
  }
